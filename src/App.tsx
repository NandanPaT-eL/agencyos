  ...
  return (
    <main className="shell">
      ...
      <section className="workspace">
        ...
        {view === 'Dashboard' && <Dashboard data={data} metrics={metrics} onSelectProject={(id) => { setSelectedProjectId(id); setView('Projects'); }} />}
...
function Dashboard({ data, metrics, onSelectProject }: { data: AppData; metrics: ReturnType<typeof calculateMetrics>; onSelectProject: (id: string) => void }) {
  const stats = [
  ...
  return <><section className="hero"><div><p className="eyebrow">MOCO + Trello + Clockify core</p><h2>Run consulting delivery from project cockpit to reports.</h2><p>Track project health, assign ticket work, log billable time, and understand where the agency spends effort.</p></div><div className="heroCard"><TimerReset/><strong>Today’s useful loop</strong><span>Create ticket → assign colleague → log time → review budget and reports.</span></div></section><section className="stats">{stats.map(({ label, value, delta, icon: Icon }) => <article className="stat" key={label}><Icon/><span>{label}</span><strong>{value}</strong><small>{delta}</small></article>)}</section><section className="grid"><article className="panel wide"><PanelTitle eyebrow="Project cockpit" title="Active projects" />{data.projects.map((project) => <button className="projectButton" key={project.id} onClick={() => onSelectProject(project.id)}><ProjectSummary data={data} project={project}/></button>)}</article><article className="panel"><PanelTitle eyebrow="Ticket system" title="My urgent work" />{data.tickets.filter((ticket) => ticket.priority === 'Urgent' || ticket.priority === 'High').map((ticket) => <TicketCard data={data} ticket={ticket} key={ticket.id}/>)}</article><article className="panel"><PanelTitle eyebrow="Time tracking" title="Recent entries" />{data.timeEntries.slice(0, 4).map((entry) => <TimeRow data={data} entry={entry} key={entry.id}/>)}</article></section></>;
}
...
function ReportsView({ data, metrics, onExport }: { data: AppData; metrics: ReturnType<typeof calculateMetrics>; onExport: () => void }) {
  return <section className="grid"><article className="panel wide"><PanelTitle eyebrow="Reports" title="Project profitability and hours" action="Export CSV" onAction={onExport}/>{data.projects.map((project) => <div className="reportRow" key={project.id}><div><strong>{project.name}</strong><small>{customerName(data, project.customerId)}</small></div><span>{projectHours(data, project.id)}h total</span><span>{projectBillableHours(data, project.id)}h billable</span><b>{formatCurrency(projectRevenue(data, project.id))}</b></div>)}</article><article className="panel"><Download/><h3>Report summary</h3><p>{metrics.totalHours}h tracked, {metrics.billableHours}h billable, {formatCurrency(metrics.revenue)} earned from logged work.</p></article></section>;
}

function CustomersView({ data, onNew }: { data: AppData; onNew: () => void }) {
  return <section className="panel"><PanelTitle eyebrow="Customers" title="Accounts and delivery" action="New customer" onAction={onNew}/><div className="cardGrid">{data.customers.map((customer) => <article className="accountCard" key={customer.id}><CustomerRow customer={customer} open={data.projects.filter((project) => project.customerId === customer.id).length}/><small>Owner: {customer.owner} · Target: {formatCurrency(customer.revenueTarget)}</small></article>)}</div></section>;
}

function TeamView({ data, onNew }: { data: AppData; onNew: () => void }) {
  return <section className="panel"><PanelTitle eyebrow="Team" title="Colleagues and workload" action="New colleague" onAction={onNew}/><div className="peopleGrid">{data.colleagues.map((person) => <article className="personCard" key={person.id}><PersonRow person={person}/><div className="meter"><i style={{ width: `${person.capacity}%` }} /></div><small>{person.billableRate}€/h · {data.tickets.filter((ticket) => ticket.assigneeId === person.id && ticket.status !== 'Done').length} open tickets · {data.timeEntries.filter((entry) => entry.colleagueId === person.id).reduce((sum, entry) => sum + entry.hours, 0)}h logged</small></article>)}</div></section>;
}
...
function TimeFields({ data, selectedProjectId }: { data: AppData; selectedProjectId: string }) { const projectTickets = data.tickets.filter((ticket) => ticket.projectId === selectedProjectId); return <><label>Project<select name="projectId" defaultValue={selectedProjectId}>{data.projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></label><label>Ticket<select name="ticketId"><option value="">No ticket</option>{projectTickets.map((ticket) => <option value={ticket.id} key={ticket.id}>{ticket.title}</option>)}</select></label><label>Person<select name="colleagueId">{data.colleagues.map((person) => <option value={person.id} key={person.id}>{person.name}</option>)}</select></label><div className="formGrid"><label>Date<input name="date" type="date" defaultValue={todayPlus(0)} /></label><label>Hours<input name="hours" type="number" min="0.25" step="0.25" defaultValue="1" /></label></div><label className="checkbox"><input name="billable" type="checkbox" defaultChecked />Billable</label><label>Note<textarea name="note" placeholder="What did you work on?" /></label></>; }
...
function ProjectSummary({ data, project }: { data: AppData; project: Project }) { return <div className="projectSummary"><div><strong>{project.name}</strong><span>{customerName(data, project.customerId)} · Lead {leadName(data, project.leadId)}</span></div><em className={statusClass(project.status)}>{project.status}</em><small>{projectHours(data, project.id)}h / {formatCurrency(project.budget)}</small></div>; }
